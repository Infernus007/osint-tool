...
export default function OSINTFeatures() {
  ...
  // Enhanced path length transforms for bright glowing beam effect
  // Start showing beams early and complete them before the content section
  const pathLengthFirst = useTransform(scrollYProgress, [0.1, 0.6], [0, 1.2]);
  const pathLengthSecond = useTransform(scrollYProgress, [0.15, 0.65], [0, 1.2]);
  const pathLengthThird = useTransform(scrollYProgress, [0.2, 0.7], [0, 1.2]);
  const pathLengthFourth = useTransform(scrollYProgress, [0.25, 0.75], [0, 1.2]);
  const pathLengthFifth = useTransform(scrollYProgress, [0.3, 0.8], [0, 1.2]);

  return (
    <div className="relative bg-gradient-to-b from-black via-slate-900 to-black min-h-[300vh]" ref={ref}>
      ...
      <GoogleGeminiEffect
        ...
        description="Experience the power of advanced intelligence gathering with our cutting-edge platform"
